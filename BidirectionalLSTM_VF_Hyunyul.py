# -*- coding: utf-8 -*-
"""
Created on Fri Jun 22 14:26:55 2018
...
# read corpus.pkl
with open('D:/Dropbox/Dropbox/DMQA_최현율/★Text mining/corpus1.pickle', 'rb') as f:
...
vector_size=64   # vector size 64로 조정함.  15로 하면 안됨.15로 조정함. 그래야 나중에 30 됨.
...
#100개의 기사를 2개씩 50일에 해당하는 tag를 붙였으니 embed된 day matrix는 row=50=day의 개수, 
#col=60= dm_vector_30+dbow_vector30이 됩니다.
print(day_matrix.shape)
print(len(day_matrix))
print(day_matrix[0:5])

# read APT_price_index.csv
...
#한 달은 30.4375일의 반올림값으로 정의했습니다. (1년을 365.25일이라고 했을 때의 평균 한달값)
...
print(np.shape(seoul_day_label))  

# Flow Matrix
n_flow=30   #30인 것을 128로 바꿈.
...
Y_LSTM_train=[]
for i in range(3000):
    Y_LSTM_train.append(y[i:i+300,:])
    
Y_LSTM_test=[]
for i in range(300):
    Y_LSTM_test.append(y[i+3000:i+3300,:])

Y_LSTM_train=np.array(Y_train)
Y_LSTM_test=np.array(Y_test)

#X_train = np.array(X_train)
#X_train_day = np.array(X_train_day)
#X_test_day = np.array(X_test_day)
#X_train_day = sequence.pad_sequences(X_train_day, maxlen=maxlen)
#X_test_day = sequence.pad_sequences(X_test_day, maxlen=maxlen)
...
max_features = 3000
embedding_size = 100
hidden_size = 64 #64로 올려도 될 듯.
batch_size = 10
maxlen = 100

# model
model = Sequential()
model.add(Bidirectional(LSTM(64, return_sequences=True), input_shape=(30, 128)))
model.add(Bidirectional(LSTM(64)))
model.add(Dense(3))
model.add(Activation('softmax'))  #1.softmax: 0.2467  2.sigmoid: 0.2567 3.ReLU: 0.3067 (배치5) 0.2467 (배치1)
model.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
print(model.summary())

print('Train...')
model.fit(X_train, Y_LSTM_train, batch_size=batch_size, epochs=100,
          validation_data=(X_test, Y_LSTM_test))  #배치사이즈=5 인데 변경해볼 것.
