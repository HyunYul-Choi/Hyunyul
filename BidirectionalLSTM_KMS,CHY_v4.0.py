...
# read corpus.pkl
with open('D:/Dropbox/Dropbox/DMQA_최현율/★Text mining/corpus.pickle', 'rb') as f:
...
max_features = 4073
maxlen = 30 #여기를 3으로 하면 두 번째 차원의 Shape은 맞으나 Dense에서 에러가 남.
...
Y_LSTM_train=[]
for i in range(3000):
    Y_LSTM_train.append(y[i:i+30,:])
    
Y_LSTM_test=[]
for i in range(866):
    Y_LSTM_test.append(y[i+3000:i+3030,:])

Y_LSTM_train=np.array(Y_LSTM_train)
Y_LSTM_test=np.array(Y_LSTM_test)

#Y_LSTM_train=np.reshape(Y_LSTM_train,(3000,1,30))
#Y_LSTM_test=np.reshape(Y_LSTM_test,(866,1,30))
...
model.add(Embedding(max_features, 256, input_length=maxlen))  # dfdfdfdf
model.add(Bidirectional(LSTM(64, return_sequences=True)))
model.add(Dropout(0.25))
model.add(Dense(3, activation='sigmoid'))  # dfdfdf
model.summary()
...
model.fit(X_train_LSTM, Y_LSTM_train, batch_size=batch_size, epochs=100,
          validation_data=[X_test_LSTM, Y_LSTM_test])
...
# Save the model
filepath = 'D:/Dropbox/Dropbox/DMQA_최현율/★Text mining/BidirectionalLSTM.h5'
model.save(filepath)